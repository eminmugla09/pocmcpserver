...
const matchPropertyToCustomerHandler = async ({ address, owner_name }: any) => {
...
const upsertOutageStatusHandler = async ({ outage_event_id, premise_number, status, cause, estimated_restoration_at, actual_restoration_at, affected_customers }: any) => {
  ...
  for (const account of accountResult.rows) {
    ...
    if (supportCase && supportCase.subject.includes("no matching outage found")) {
      ...
      await pool.query(
        `UPDATE support_cases SET subject = $1, description = $2, updated_at = CURRENT_TIMESTAMP WHERE case_id = $3`,
        [newSubject, newDescription, supportCase.case_id]
      );
    }
...
const getCaseStatusHandler = async ({ case_id }: any) => {
  ...
  if (supportCase.category?.toLowerCase() === "outage") {
    const accountResult = await pool.query(
      `SELECT premise_number FROM accounts WHERE account_number = $1`,
      [supportCase.account_number]
    );
    const premiseNumber = accountResult.rows[0]?.premise_number;
    let outage = null;
    if (premiseNumber) {
      const outageResult = await pool.query(
        `SELECT * FROM outage_events WHERE premise_number = $1 ORDER BY updated_at DESC LIMIT 1`,
        [premiseNumber]
      );
...
registerAppTool(server, "render_usage_chart", {
  ...
  content: [{ type: "text" as const, text: `Showing usage chart for ${args.months.length} months.` }]
}));
...
registerAppTool(server, "render_billing_dashboard", {
  ...
  content: [{ type: "text" as const, text: `Showing billing dashboard. Current bill: $${args.current_bill.total}, due ${args.current_bill.due_date}.` }]
}));
...
registerAppTool(server, "render_high_bill_explanation", {
  ...
  structuredContent: args,
  content: [{ type: "text" as const, text: `Bill analysis: ${args.current_month} bill is $${args.current_bill} (${args.bill_increase_pct > 0 ? '+' : ''}${args.bill_increase_pct}% vs previous).` }]
}));
...
registerAppTool(server, "render_rate_comparison", {
  ...
  structuredContent: args,
  content: [{ type: "text" as const, text: `Rate comparison: Current plan ${args.current_plan} at $${args.current_monthly_avg}/mo. ${args.comparisons.length} alternatives analyzed.` }]
}));
...
registerAppTool(server, "render_outage_status", {
  ...
  structuredContent: args,
  content: [{ type: "text" as const, text: `Outage status: ${args.status}${args.estimated_restoration_at ? `, ETA: ${args.estimated_restoration_at}` : ''}.` }]
}));
...
registerAppTool(server, "render_ev_charging_chart", {
  ...
  structuredContent: args,
  content: [{ type: "text" as const, text: `Showing EV charging data${args.vehicle ? ` for ${args.vehicle}` : ''}.${args.total_savings_ytd ? ` YTD savings: $${args.total_savings_ytd}` : ''}` }]
}));
...
registerAppTool(server, "render_account_overview", {
  ...
  structuredContent: args,
  content: [{ type: "text" as const, text: `Account overview for ${args.customer_name} (${args.account_number}). Status: ${args.account_status}, Standing: ${args.account_standing}.` }]
}));
...
const summarizeForLog = (value: unknown, maxChars = MAX_LOG_BODY_CHARS) => {
  let text: string;
  if (typeof value === "string") {
    text = value;
  } else {
...
const parseSseResponseBody = (body: string) => {
...
const attachResponseBodyCapture = (response: ServerResponse) => {
  const chunks: Buffer[] = [];

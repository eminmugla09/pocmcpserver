...
describe('Remaining Handler Tests', () => {
  ...
  describe('Proactive Notifications', () => {
    it('should subscribe and create bill threshold notifications through scheduled checks', async () => {
      ...
      expect(notifications.some((notification: any) => notification.eventType === 'bill_projection_threshold')).toBe(true);
    });
    ...
    it('should create outage restoration notifications with support case update from event updates', async () => {
      ...
      const outageNotifications = notifications.filter((notification: any) => notification.eventType === 'outage_restoration');
    ...
    it('should create outage restoration notifications with support case update through scheduled checks', async () => {
      ...
      await mainPool.query(
        `INSERT INTO proactive_notification_subscriptions (customer_number, account_number, monitor_type, channel, frequency_minutes, enabled)
         VALUES ($1, $2, $3, $4, $5, TRUE)`,
        ['1009988776', '5210099001', 'outage_restoration', 'email', 1]
      );
      ...
      expect(check.status).toBe('CHECKED');
      const outageNotification = check.notifications.find((notification: any) => notification.event_type === 'outage_restoration');

...
describe('HTTP Transport Layer Tests', () => {
  ...
  describe('OAuth Branch Coverage', () => {
    const issueAuthorizationCode = async (clientId: string, redirectUri: string, state = 's1') => {
      const response = await fetch(
        `${baseUrl}/oauth/authorize?client_id=${encodeURIComponent(clientId)}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&state=${encodeURIComponent(state)}&code_challenge=plain-challenge&code_challenge_method=plain`,
        {
          method: 'POST',
          redirect: 'manual',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    it('should render oauth login page for valid client', async () => {
      const response = await fetch(
        `${baseUrl}/oauth/authorize?client_id=${encodeURIComponent(dynamicClientId)}&response_type=code&redirect_uri=${encodeURIComponent('https://example.com/callback')}&state=abc`
      );
      const html = await response.text();
    ...
    it('should reject authorize request with invalid redirect uri', async () => {
      const response = await fetch(
        `${baseUrl}/oauth/authorize?client_id=${encodeURIComponent(dynamicClientId)}&response_type=code&redirect_uri=${encodeURIComponent('https://evil.example/callback')}`
      );
      const html = await response.text();
    ...
    it('should reject authorize login when credentials are missing', async () => {
      const response = await fetch(
        `${baseUrl}/oauth/authorize?client_id=${encodeURIComponent(dynamicClientId)}&response_type=code&redirect_uri=${encodeURIComponent('https://example.com/callback')}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: ''
    ...
    it('should issue auth code and exchange for token with PKCE plain method', async () => {
      const authorizeResponse = await fetch(
        `${baseUrl}/oauth/authorize?client_id=${encodeURIComponent(dynamicClientId)}&response_type=code&redirect_uri=${encodeURIComponent('https://example.com/callback')}&state=xyz&code_challenge=plain-challenge&code_challenge_method=plain`,
        {
          method: 'POST',
          redirect: 'manual',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      ...
      const goodAuthorizeResponse = await fetch(
        `${baseUrl}/oauth/authorize?client_id=${encodeURIComponent(dynamicClientId)}&response_type=code&redirect_uri=${encodeURIComponent('https://example.com/callback')}&state=xyz2&code_challenge=plain-challenge&code_challenge_method=plain`,
        {
          method: 'POST',
          redirect: 'manual',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      ...
      const goodCode = new URL(goodLocation).searchParams.get('code') as string;
    ...
    it('should reject token exchange when authorization code belongs to another client', async () => {
      ...
      const secondClientId = secondClientPayload.client_id as string;
      const secondClientSecret = secondClientPayload.client_secret as string;
    ...
    it('should reject revoked and expired refresh tokens on refresh endpoint', async () => {
      ...
      await mainPool.query(
        `INSERT INTO oauth_refresh_tokens (refresh_token, client_id, user_id, email, expires_at, revoked)
         VALUES ($1, $2, (SELECT id FROM users WHERE email = $3), $3, NOW() + INTERVAL '1 day', TRUE),
                ($4, $2, (SELECT id FROM users WHERE email = $3), $3, NOW() - INTERVAL '1 day', FALSE)`,
        [revokedToken, defaultClientId, 'woarzus@gmail.com', expiredToken]
      );

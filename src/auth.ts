...
export async function linkUserToCustomer(
  ...
  try {
    await pool.query(
      ...
      [userId, customerNumber, isPrimary]
    );
...
export async function getPrimaryCustomerNumber(userId: string): Promise<string | null> {
  try {
    const result = await pool.query(
      'SELECT customer_number FROM user_customers WHERE user_id = $1 AND is_primary = TRUE LIMIT 1',
      [userId]
    );

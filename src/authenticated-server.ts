...
async function checkAccess(userId: string, customerNumber: string): Promise<boolean> {
  return await hasCustomerAccess(userId, customerNumber);
}

// Helper function to get account's customer number
async function getAccountCustomer(accountNumber: string): Promise<string | null> {
  try {
...
async function getPremiseCustomer(premiseNumber: string): Promise<string | null> {
  try {
    const result = await pool.query(
      'SELECT customer_number FROM accounts WHERE premise_number = $1 LIMIT 1',
      [premiseNumber]
    );
    return result.rows.length > 0 ? result.rows[0].customer_number : null;
  } catch (error) {
  ...
  try {
    switch (name) {
      ...
      case 'get_payment_history': {
        ...
        const result = await pool.query(
          'SELECT * FROM payment_history WHERE account_number = $1 ORDER BY payment_date DESC LIMIT 10',
          [account_number]
        );
      ...
      case 'get_usage_history': {
        ...
        const result = await pool.query(
          'SELECT * FROM usage_history WHERE account_number = $1 ORDER BY month DESC LIMIT 12',
          [account_number]
        );

...
const getPremiseDetailsHandler = async ({ premise_number, address }) => {
    ...
    const nextAction = serviceActive
        ...
        : `Service is inactive at premise ${premise.premise_number}. You MUST offer the customer to schedule move-in service. Use the address and date from the public records tool. If public records has a closing/move-in/renting date, say: "I can schedule FPL electric service at [address] to start on [date]. Would you like me to do that? We can't schedule EV installation until power is active." If no date is available, ask: "What date would you like FPL electric service to start at [address]?" Then call schedule_move_in_service with premise_number="${premise.premise_number}" and requested_connect_date set to that date.`;
    const customerOfferTemplate = serviceActive
        ? ""
        : "I can schedule FPL electric service at [address] to start on [date]. Would you like me to do that? We can't schedule EV installation until power is active.";
    return {
        ...premise,
        serviceActive,
        nextAction,
...
const checkEvEligibilityHandler = async ({ premise_number }) => {
    ...
    const nextAction = serviceActive
        ...
        : `Service is inactive. You MUST offer the customer to schedule move-in service. Use the address and date from the public records tool. If public records has a closing/move-in/renting date, say: "I can schedule FPL electric service at [address] to start on [date], then set up the EV charger after power is active. Would you like me to schedule the service now?" If no date is available, ask: "What date would you like FPL electric service to start at [address]?" Then call schedule_move_in_service with premise_number="${premise_number}" and requested_connect_date set to that date. Do NOT offer or call schedule_ev_assessment or enroll_ev_charging until power is active.`;
    const customerOfferTemplate = serviceActive
        ? ""
        : "I can schedule FPL electric service at [address] to start on [date], then set up the EV charger after power is active. Would you like me to schedule the service now?";
    return {
        ...eligibility,
        serviceStatus,
        serviceActive,

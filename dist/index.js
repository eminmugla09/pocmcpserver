...
const createFplMcpServer = () => {
    ...
    server.registerTool("get_premise_details", {
        description: "Return FPL premise details by premise number or service address. Includes property type, service status, garage readiness (240V circuit, WiFi), and EV suitability. Does NOT include public-property-record data (closing date, sale price, parcel ID, etc.) — get those from the public-property records connector. When service is inactive, the response tells you to call schedule_move_in_service with the returned premise_number and the closing/move-in date from the public-property records. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Use after resolving the address through public-property records (e.g., for a home purchase) or direct customer input. For EV questions, call this and check_ev_eligibility together in sequence.",
        inputSchema: {
            premise_number: z.string().optional(),
            address: z.string().optional()
        }
    ...
    server.registerTool("match_property_to_customer", {
        description: "Link a known new-property street address to the existing FPL customer and premise. Pass owner_name from the public-property records connector when available; if the premise has no active account, the tool uses owner_name to find the matching FPL customer. Does NOT return public-record details — get those from the public-property records connector. Use after an available public-property records connector/tool finds a recent property event, or when the customer directly provides the new address. If no public-property connector/tool is available, ask the customer for the street address and their full name.",
        inputSchema: {
            address: z.string(),
    ...
    if (body?.method === "tools/list") {
        ...
        try {
            ...
            const tools = [
                ...
                { name: "match_property_to_customer", description: "Link a known new-property street address to the existing FPL customer and premise. Pass owner_name from the public-property records connector when available; if the premise has no active account, the tool uses owner_name to find the matching FPL customer. Does NOT return public-record details — get those from the public-property records connector. Use after an available public-property records connector/tool finds a recent property event, or when the customer directly provides the new address. If no public-property connector/tool is available, ask the customer for the street address and their full name.", inputSchema: { type: "object", properties: { address: { type: "string" }, owner_name: { type: "string" } }, required: ["address"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },

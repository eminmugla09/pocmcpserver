...
const reportOutageHandler = async ({ account_number, description }) => {
    ...
    return {
        ...
        scheduledCheckOffer: "Would you like me to set up ChatGPT scheduled checks for restoration updates and notify you when there is a new outage status?",
        scheduledCheckTool: "subscribe_proactive_notifications",
        scheduledCheckInput: {
            customer_number: account.customer_number,
            account_number,
            monitor_type: "outage_restoration",
            channel: "email",
            frequency_minutes: 60
        },
        nextAction: "You MUST ask the customer: 'Would you like me to set up ChatGPT scheduled checks for restoration updates and notify you when there is a new outage status?' If yes, call subscribe_proactive_notifications with monitor_type='outage_restoration'."
    };
};
...
const createFplMcpServer = () => {
    ...
    server.registerTool("report_outage", {
        description: "Use this when a customer says they have an outage, power is out, lights are out, or asks to report an outage. Checks known outage records for the customer's service address and nearby service area, creates a high-priority outage support case either way, returns any known restoration estimate, and includes scheduledCheckOffer. After reporting the outage, you MUST ask the customer if they want ChatGPT scheduled restoration-status checks. If they say yes, call subscribe_proactive_notifications with monitor_type='outage_restoration'.",
        inputSchema: {
            account_number: z.string(),

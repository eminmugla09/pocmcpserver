...
const getPremiseDetailsHandler = async ({ premise_number, address }) => {
    ...
    const nextAction = serviceActive
        ...
        : `Service is inactive at premise ${premise.premise_number}. Ask the customer: "I can schedule FPL electric service at this address to start on your preferred date. If you have a closing or move-in date from public records, I'll offer that day; otherwise, what date would you like?" Then call schedule_move_in_service with premise_number="${premise.premise_number}" and requested_connect_date set to the customer's preferred date.`;
    const customerOffer = serviceActive
        ? ""
        : "I can schedule FPL electric service at this address to start on your preferred date. If you have a closing or move-in date, I'll offer that day; otherwise, what date would you like?";
    return {
        ...premise,
        serviceActive,
        nextAction,
...
const checkEvEligibilityHandler = async ({ premise_number }) => {
    ...
    const nextAction = serviceActive
        ...
        : `Service is inactive. Ask the customer: "I can schedule FPL electric service at this address to start on your preferred date. If you have a closing or move-in date from public records, I'll offer that day; otherwise, what date would you like? Then I'll set up the EV charger after power is active." Then call schedule_move_in_service with premise_number="${premise_number}" and requested_connect_date set to the customer's preferred date. Do NOT offer or call schedule_ev_assessment or enroll_ev_charging until power is active.`;
    const customerOffer = serviceActive
        ? ""
        : "I can schedule FPL electric service at this address to start on your preferred date. If you have a closing or move-in date, I'll offer that day; otherwise, what date would you like? Then I'll set up the EV charger after power is active.";
    return {
        ...eligibility,
        serviceStatus,
        serviceActive,
...
const createFplMcpServer = () => {
    ...
    server.registerTool("get_premise_details", {
        description: "Return FPL premise details by premise number or service address. Includes property type, service_status, serviceActive (boolean), garage readiness (240V circuit, WiFi), EV suitability, and concrete nextAction/instructions/customerOffer fields. When serviceActive is false, the customerOffer field contains the exact sentence to say to the customer to offer schedule_move_in_service. Use that sentence verbatim. If a public records tool provides a closing or move-in date, offer that day; otherwise ask the customer for their preferred date. Ask for confirmation before calling schedule_move_in_service. Does NOT include public-record data (closing date, sale price, parcel ID, etc.) — get those from a public records tool. Use after resolving the address through a public records tool (e.g., for a home purchase) or direct customer input. For EV questions, call this and check_ev_eligibility together in sequence.",
        inputSchema: {
            premise_number: z.string().optional(),
            address: z.string().optional()
        }
    ...
    server.registerTool("check_ev_eligibility", {
        description: "Return premise-specific FPL EVolution Home eligibility checks and recommended install type. The response includes recommended_install_type, alternate_install_type, serviceActive (true/false), nextAction, instructions, and customerOffer. The customerOffer field contains the exact sentence to say to the customer when service is inactive — use it verbatim to offer schedule_move_in_service. If a public records tool provides a closing or move-in date, offer that day; otherwise ask the customer for their preferred date. Ask for confirmation before calling the tool. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Display both EV plans to the customer. Requires premise_number. Call immediately after get_premise_details for the same premise.",
        inputSchema: {
    ...
    if (body?.method === "tools/list") {
        ...
        try {
            ...
            const tools = [
                ...
                { name: "check_ev_eligibility", description: "Check whether a specific premise is eligible for FPL EVolution Home EV charging enrollment. Returns eligibility status, recommended_install_type, alternate_install_type, serviceActive, nextAction, instructions, and customerOffer. The customerOffer field contains the exact sentence to say to the customer when service is inactive — use it verbatim to offer schedule_move_in_service. If a public records tool provides a closing or move-in date, offer that day; otherwise ask the customer for their preferred date. Ask for confirmation before calling the tool. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Display both EV plans to the customer. Requires premise_number. Call immediately after get_premise_details for the same premise.", inputSchema: { type: "object", properties: { premise_number: { type: "string" } }, required: ["premise_number"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
                ...
                { name: "schedule_move_in_service", description: "Schedule a move-in electric service start date for a resolved premise. Requires premise_number and requested_connect_date (the closing or move-in date). This is the PRIMARY tool to use when a customer is purchasing a home and needs power turned on by a specific date. It is idempotent and returns an existing SUBMITTED order if one already exists. Use the closing date from any available public-property records tool as the requested_connect_date, or the customer's move-in date. Only call after the customer confirms the address and the date.", inputSchema: { type: "object", properties: { premise_number: { type: "string" }, account_number: { type: "string" }, requested_connect_date: { type: "string" } }, required: ["premise_number", "requested_connect_date"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },

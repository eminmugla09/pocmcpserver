...
const createFplMcpServer = () => {
    ...
    server.registerTool("get_premise_details", {
        description: "Return FPL premise details by premise number or service address. Includes property type, service status, garage readiness (240V circuit, WiFi), and EV suitability. Does NOT include public-property-record data (closing date, sale price, parcel ID, etc.) — get those from any available public-property records tool. When service is inactive, the response tells you to call schedule_move_in_service with the returned premise_number and the closing/move-in date from a public records tool. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Use after resolving the address through a public records tool (e.g., for a home purchase) or direct customer input. For EV questions, call this and check_ev_eligibility together in sequence.",
        inputSchema: {
            premise_number: z.string().optional(),
            address: z.string().optional()
        }
    ...
    if (body?.method === "tools/list") {
        ...
        try {
            ...
            const tools = [
                ...
                { name: "get_premise_details", description: "Return FPL premise details for a service address or premise number. Includes property type, service status, garage readiness (240V circuit, WiFi), and EV suitability. Does NOT include public-property-record data (closing date, sale price, parcel ID, etc.) — get those from any available public-property records tool. When service is inactive, the response tells you to call schedule_move_in_service with the returned premise_number and the closing/move-in date from the public records tool. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Use after resolving the address through a public records tool (e.g., for a home purchase) or direct customer input. For EV questions, call this and check_ev_eligibility together in sequence.", inputSchema: { type: "object", properties: { premise_number: { type: "string" }, address: { type: "string" } }, additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },

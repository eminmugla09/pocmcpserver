...
const getPremiseDetailsHandler = async ({ premise_number, address }) => {
    ...
    const nextAction = serviceActive
        ...
        : `Service is inactive at premise ${premise.premise_number}. Proactively offer the customer to schedule move-in electric service with schedule_move_in_service, using the closing/move-in date from the public records tool as requested_connect_date. Do NOT offer or call schedule_ev_assessment or enroll_ev_charging until power is active.`;
    return {
        ...premise,
        serviceActive,
        nextAction,
...
const checkEvEligibilityHandler = async ({ premise_number }) => {
    ...
    const nextAction = serviceActive
        ...
        : `Service is inactive. Proactively offer the customer to schedule move-in electric service with schedule_move_in_service, using the closing/move-in date from the public records tool as requested_connect_date. Do NOT offer or call schedule_ev_assessment or enroll_ev_charging until power is active. After the customer confirms, call schedule_move_in_service and then proceed with enroll_ev_charging after activation.`;
    return {
        ...eligibility,
        serviceStatus,
        serviceActive,
...
const createFplMcpServer = () => {
    ...
    server.registerTool("get_premise_details", {
        description: "Return FPL premise details by premise number or service address. Includes property type, service_status, serviceActive (boolean), garage readiness (240V circuit, WiFi), EV suitability, and a concrete nextAction/instructions field. The nextAction explicitly tells you whether to proactively offer schedule_move_in_service (with the closing/move-in date from a public records tool) or to proceed with EV eligibility. Does NOT include public-property-record data (closing date, sale price, parcel ID, etc.) — get those from any available public-property records tool. Use after resolving the address through a public records tool (e.g., for a home purchase) or direct customer input. For EV questions, call this and check_ev_eligibility together in sequence.",
        inputSchema: {
            premise_number: z.string().optional(),
            address: z.string().optional()
        }
    ...
    server.registerTool("check_ev_eligibility", {
        description: "Return premise-specific FPL EVolution Home eligibility checks and recommended install type. The response includes recommended_install_type, alternate_install_type, serviceActive (true/false), nextAction, and instructions. The nextAction is explicit: if serviceActive is false, proactively offer the customer to schedule move-in service with schedule_move_in_service using the closing/move-in date from a public records tool as requested_connect_date. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Display both EV plans to the customer. Requires premise_number. Call immediately after get_premise_details for the same premise.",
        inputSchema: {
    ...
    if (body?.method === "tools/list") {
        ...
        try {
            ...
            const tools = [
                ...
                { name: "check_ev_eligibility", description: "Check whether a specific premise is eligible for FPL EVolution Home EV charging enrollment. Returns eligibility status, recommended_install_type, alternate_install_type, serviceActive, nextAction, and instructions. The nextAction is explicit: if serviceActive is false, proactively offer the customer to schedule move-in service with schedule_move_in_service using the closing/move-in date from a public records tool as requested_connect_date. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Display both EV plans to the customer. Requires premise_number. Call immediately after get_premise_details for the same premise.", inputSchema: { type: "object", properties: { premise_number: { type: "string" } }, required: ["premise_number"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },

...
const getMyAccountOverviewHandler = async (userId, email) => {
    // Get all linked accounts via user_customers
    const ucResult = await pool.query(`SELECT uc.customer_number, uc.is_primary, a.account_number, a.premise_number, a.status, a.standing, a.rate_class, a.past_due_flag, a.smart_meter_flag, a.service_address_line1, a.service_address_city, a.service_address_state, a.service_address_zip
     FROM user_customers uc
     INNER JOIN accounts a ON a.customer_number = uc.customer_number
     WHERE uc.user_id = $1
     ORDER BY uc.is_primary DESC, a.account_number ASC`, [userId]);
    ...
    const accountsData = await Promise.all(ucResult.rows.map(async (acct) => {
...
const getPremiseDetailsHandler = async ({ premise_number, address }) => {
    ...
    const pendingOrder = !serviceActive
        ? (await pool.query(`SELECT * FROM service_connection_orders
         WHERE premise_number = $1 AND status = 'SUBMITTED'
         ORDER BY created_at DESC LIMIT 1`, [premise.premise_number])).rows[0]
        : null;
    const nextAction = serviceActive
        ...
        : pendingOrder
            ...
            : `Service is inactive for this address. You MUST first congratulate the customer and offer to schedule move-in electric service using the address and date from the public records tool. Say: "Congratulations on your new home! I can schedule FPL electric service at [address] to start on [date]. Would you like me to do that? We can't schedule EV installation until power is active." If no date is available, ask for the preferred service start date. Do not ask about existing active service in the same first offer. If the customer says yes and get_my_account_overview shows another active service address, then ask: "You also currently have active FPL service at [existing_service_address]. Do you want to keep that service active, or are you moving from that address?" If the customer says they are moving from that address, ask: "What date would you like service to stop at [existing_service_address]?" Only call schedule_move_in_service after the customer confirms the new address, new service start date, existing-service choice, and move-out date when they are moving from the existing address. Do not say "premise" or show internal premise numbers to the customer. Use premise_number="${premise.premise_number}" only as the internal tool argument.`;
...
const checkEvEligibilityHandler = async ({ premise_number }) => {
    ...
    const premiseResult = await pool.query('SELECT service_status FROM premises WHERE premise_number = $1', [premise_number]);
    ...
    const pendingOrder = !serviceActive
        ? (await pool.query(`SELECT * FROM service_connection_orders
         WHERE premise_number = $1 AND status = 'SUBMITTED'
         ORDER BY created_at DESC LIMIT 1`, [premise_number])).rows[0]
        : null;
    const nextAction = serviceActive
...
const matchPropertyToCustomerHandler = async ({ address, owner_name }) => {
...
const scheduleMoveInServiceHandler = async (input) => {
    if (!input.requested_connect_date) {
        return {
            ...
            moveIntentQuestion: "After the customer agrees to schedule service, if they have active FPL service at another address, ask before scheduling: 'You also currently have active FPL service at [existing_service_address]. Do you want to keep that service active, or are you moving from that address?' If the customer says they are moving from that address, ask: 'What date would you like service to stop at [existing_service_address]?'",
...
const setMoveIntentHandler = async (input) => {
    const moveOutDateText = input.move_out_date ? ` Requested move-out date: ${input.move_out_date}.` : "";
    const message = input.intent === "keep_both"
        ...
        : `Noted that you intend to move out of an existing address.${moveOutDateText} No stop-service order is created until the existing address and stop date are explicitly confirmed.`;
...
const setAutopayHandler = async ({ account_number, payment_method }) => {
    ...
    const updateResult = await pool.query(`UPDATE billing
     SET autopay_enrolled = TRUE,
         next_scheduled_payment_date = $2,
         next_scheduled_payment_amount_usd = $3,
         updated_at = CURRENT_TIMESTAMP
     WHERE account_number = $1
     RETURNING account_number`, [account_number, latestBill?.due_date || null, latestBill?.amount_due || null]);
...
const cancelAutopayHandler = async ({ account_number }) => {
    const updateResult = await pool.query(`UPDATE billing
     SET autopay_enrolled = FALSE,
         ...
         updated_at = CURRENT_TIMESTAMP
     WHERE account_number = $1
     RETURNING account_number`, [account_number]);
    if (updateResult.rowCount === 0) {
        return {
            status: "NOT_FOUND",
            accountNumber: account_number,
...
const requestPaymentExtensionHandler = async ({ account_number, requested_due_date, reason }) => {
    const updateResult = await pool.query(`UPDATE billing
     ...
     RETURNING account_number`, [account_number, requested_due_date]);
    if (updateResult.rowCount === 0) {
        return {
            status: "NOT_FOUND",
            accountNumber: account_number,
...
const createProactiveNotification = async ({ subscriptionId, customerNumber, accountNumber, eventType, title, message, severity, payload, dedupeKey }) => {
    const result = await pool.query(`INSERT INTO proactive_notifications
...
const subscribeProactiveNotificationsHandler = async ({ customer_number, account_number, monitor_type, channel, frequency_minutes, threshold_usd }) => {
    const result = await pool.query(`INSERT INTO proactive_notification_subscriptions
      (customer_number, account_number, monitor_type, channel, frequency_minutes, threshold_usd, enabled)
     VALUES ($1, $2, $3, $4, $5, $6, TRUE)
     RETURNING id, customer_number, account_number, monitor_type, channel, frequency_minutes, threshold_usd, enabled, created_at`, [customer_number, account_number || null, monitor_type, channel || "email", frequency_minutes || 60, threshold_usd || null]);
...
const getProactiveNotificationsHandler = async ({ customer_number, account_number, status, limit }) => {
    const maxItems = Math.min(Math.max(Number(limit || 20), 1), 100);
    const result = await pool.query(`SELECT id, subscription_id, customer_number, account_number, event_type, title, message, severity, status, payload, created_at, delivered_at
     FROM proactive_notifications
     WHERE ($1::text = '' OR customer_number = $1)
       AND ($2::text = '' OR account_number = $2)
       AND ($3::text = '' OR status = $3)
     ORDER BY created_at DESC
     LIMIT $4`, [customer_number || '', account_number || '', status || '', maxItems]);
    return result.rows.map((row) => ({
        id: row.id,
...
const buildOutageStatusUpdate = async ({ customer_number, account_number }) => {
    const accountResult = await pool.query(`SELECT account_number, customer_number, premise_number, service_address_line1, service_address_city, service_address_state, service_address_zip
     FROM accounts
     WHERE ($1::text = '' OR customer_number = $1)
       AND ($2::text = '' OR account_number = $2)`, [customer_number || '', account_number || '']);
    const updates = [];
    for (const account of accountResult.rows) {
        ...
        const outageResult = await pool.query(`SELECT *
       FROM outage_events
       WHERE premise_number = $1
       ORDER BY updated_at DESC
       LIMIT 1`, [account.premise_number]);
        const outage = outageResult.rows[0] || null;
        const caseResult = await pool.query(`SELECT case_id, status, subject, priority, updated_at
       FROM support_cases
       WHERE account_number = $1 AND category = 'outage'
       ORDER BY updated_at DESC
       LIMIT 1`, [account.account_number]);
...
const getOutageStatusHandler = async ({ customer_number, account_number, case_id }) => {
    let resolvedAccountNumber = account_number;
    if (case_id) {
        const caseResult = await pool.query(`SELECT account_number FROM support_cases WHERE case_id = $1`, [case_id]);
...
const upsertOutageStatusHandler = async ({ outage_event_id, premise_number, status, cause, estimated_restoration_at, actual_restoration_at, affected_customers }) => {
    const outageEventId = outage_event_id || makeId("OUTAGE");
    const result = await pool.query(`INSERT INTO outage_events
     ...
     DO UPDATE SET status = EXCLUDED.status,
                   ...
                   updated_at = CURRENT_TIMESTAMP
     RETURNING *`, [outageEventId, premise_number, status, cause || null, estimated_restoration_at || null, actual_restoration_at || null, affected_customers || null]);
    const outage = result.rows[0];
    const accountResult = await pool.query(`SELECT account_number, customer_number, service_address_line1, service_address_city, service_address_state, service_address_zip
     FROM accounts
     WHERE premise_number = $1`, [premise_number]);
    const notifications = [];
    for (const account of accountResult.rows) {
        const serviceAddress = `${account.service_address_line1}, ${account.service_address_city}, ${account.service_address_state} ${account.service_address_zip}`;
       ...
       LIMIT 1`, [account.account_number]);
        const supportCase = caseResult.rows[0] || null;
        // Update the support case subject/description if it was created before the
        // outage event existed (stale "no matching outage found" subject).
        if (supportCase && supportCase.subject.includes("no matching outage found")) {
            const newSubject = status.toLowerCase() === "restored"
                ? "Customer outage report - power restored"
                : "Customer outage report - known outage found";
            const estimateText = outage.estimated_restoration_at ? ` Estimated restoration: ${outage.estimated_restoration_at}.` : "";
            const restoredText = outage.actual_restoration_at ? ` Restored at: ${outage.actual_restoration_at}.` : "";
            const newDescription = `Customer reported a power outage. Service address: ${serviceAddress}. Known outage status: ${status}.${estimateText}${restoredText} Outage event: ${outageEventId}.`;
            await pool.query(`UPDATE support_cases SET subject = $1, description = $2, updated_at = CURRENT_TIMESTAMP WHERE case_id = $3`, [newSubject, newDescription, supportCase.case_id]);
        }
...
const runScheduledNotificationChecksHandler = async ({ customer_number, account_number } = {}) => {
    const subscriptionResult = await pool.query(`SELECT *
     ...
     ORDER BY created_at ASC`, [customer_number || '', account_number || '']);
    const notifications = [];
    for (const subscription of subscriptionResult.rows) {
        if (subscription.monitor_type === "outage_restoration") {
            const outageResult = await pool.query(`SELECT o.*, a.account_number, a.customer_number, a.service_address_line1, a.service_address_city, a.service_address_state, a.service_address_zip
         ...
         LIMIT 5`, [subscription.customer_number, subscription.account_number]);
        ...
        if (subscription.monitor_type === "service_request_status") {
            const orderResult = await pool.query(`SELECT service_order_id, account_number, order_type, effective_date, status, updated_at, created_at
         FROM service_orders
         ...
         WHERE ($1::text IS NULL OR account_number = $1)
         ORDER BY created_at DESC
         LIMIT 10`, [subscription.account_number]);
        ...
        await pool.query(`UPDATE proactive_notification_subscriptions
...
const reportOutageHandler = async ({ account_number, description }) => {
    const accountResult = await pool.query(`SELECT account_number, customer_number, premise_number, service_address_line1, service_address_city, service_address_state, service_address_zip
     FROM accounts
     WHERE account_number = $1`, [account_number]);
    ...
    const outageResult = await pool.query(`SELECT *
     ...
     LIMIT 1`, [account.premise_number]);
    const outage = outageResult.rows[0] || null;
    const nearbyResult = await pool.query(`SELECT COUNT(*)::int AS active_outage_count
     FROM outage_events o
     INNER JOIN accounts a ON a.premise_number = o.premise_number
     WHERE LOWER(o.status) <> 'restored'
       AND a.service_address_city = $1
       AND a.service_address_state = $2`, [account.service_address_city, account.service_address_state]);
...
const createFplMcpServer = () => {
    const server = new McpServer({
        ...
        instructions: `This server exposes FPL utility tools. IMPORTANT RENDERING RULES — you MUST follow these:

After calling any of these data tools, you MUST ALWAYS immediately call the corresponding render tool to display an interactive widget. Do NOT skip the render step. Do NOT just show text.

MANDATORY render pairings:
- After get_usage_history → MUST call render_usage_chart
- After get_billing_inquiry → MUST call render_billing_dashboard
- After get_billing_inquiry + get_usage_history (for high bill questions) → MUST call render_high_bill_explanation
- After get_rate_plan_options or compare_rate_plan_savings → MUST call render_rate_comparison
- After get_outage_status or report_outage → MUST call render_outage_status
- After get_ev_charging_sessions → MUST call render_ev_charging_chart
- After get_my_account_overview → MUST call render_account_overview

The render tools display interactive charts and dashboards inline. Extract the relevant fields from the data tool response and pass them as arguments to the render tool. You may also include a brief text summary alongside the widget.`
    });
    ...
    server.registerTool("get_premise_details", {
        ...
        const summary = [
            ...
            isInactive
                ? `ACTION: Service is inactive. First congratulate the customer and offer to schedule electric service at this address for the closing/move-in date. If the customer says yes and activeServiceAddresses has another active address, then ask whether they want to keep that existing service active before calling schedule_move_in_service. If the customer says they are moving from the existing address, ask for the move-out/stop-service date before calling schedule_move_in_service. Use premise_number="${premise.premise_number}" only as an internal tool argument with requested_connect_date="[closing/move-in date from public-property records]". Do NOT schedule_ev_assessment or enroll_ev_charging yet. After service is active, call schedule_ev_assessment or enroll_ev_charging.`
    ...
    server.registerTool("get_billing_inquiry", {
        description: "Return current bill, due date, charge breakdown, kWh usage, EV off-peak savings, recent payments (last 5), and AutoPay status. This is the single tool for all billing and payment questions. After calling this, you MUST call render_billing_dashboard to display the interactive dashboard.",
        inputSchema: {
            account_number: z.string()
        }
    ...
    server.registerTool("get_usage_history", {
        description: "Return monthly kWh, cost and EV charging kWh trend history. After calling this, you MUST call render_usage_chart to display the interactive chart.",
        inputSchema: {
            account_number: z.string()
        }
    ...
    server.registerTool("check_ev_eligibility", {
        description: "Return address-specific FPL EVolution Home eligibility checks and recommended install type. The response includes recommended_install_type, alternate_install_type, serviceActive (true/false), nextAction, instructions, and customerOfferTemplate. When serviceActive is false, first congratulate the customer and offer to schedule electric service for the address and closing/move-in/renting date from the public records tool. Do not ask about existing active service in that first offer. If the customer says yes and activeServiceAddresses from get_my_account_overview includes another service address, then ask whether to keep that service active before calling schedule_move_in_service. If the customer says they are moving from that address, ask for the move-out/stop-service date before calling schedule_move_in_service. If no date is available, ask the customer for their preferred date. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Display both EV plans to the customer. Requires premise_number as an internal argument only. Call immediately after get_premise_details for the same address. Do not say 'premise' or show internal premise numbers to the customer.",
        inputSchema: {
    ...
    server.registerTool("match_property_to_customer", {
        description: "Link a known new-property street address to the existing FPL customer and premise. Pass owner_name from any available public-property records tool when available; if the premise has no active account, the tool uses owner_name to find the matching FPL customer. Does NOT return public-record details — get those from a public records tool. Use after a public records tool finds a recent property event, or when the customer directly provides the new address. If no public records tool is available, ask the customer for the street address and their full name.",
        inputSchema: {
            address: z.string(),
            owner_name: z.string().optional()
        }
    }, async ({ address, owner_name }) => {
        const result = await matchPropertyToCustomerHandler({ address, owner_name });
    ...
    server.registerTool("schedule_move_in_service", {
        description: "Schedule a move-in electric service start date for a resolved service address. Requires premise_number as an internal argument and requested_connect_date (the closing or move-in date). This is the primary tool to use when a customer is purchasing a home and needs power turned on by a specific date. It is idempotent and will return an existing SUBMITTED order if one already exists. Use the closing date from any available public-property records tool as the requested_connect_date, or the customer's move-in date. Only call after the customer first agrees to schedule service for the new service address/start date, and if activeServiceAddresses shows another active FPL service, the customer answers whether to keep that existing service active. If the customer says they are moving from the existing address, ask for the move-out/stop-service date before calling this tool. If they answer about existing service, call set_move_intent to record keep_both or move_out_existing. Do NOT stop existing service without explicit confirmation. Do not say 'premise' or show internal premise numbers to the customer.",
        inputSchema: {
            premise_number: z.string(),
            account_number: z.string().optional(),
            requested_connect_date: z.string()
        }
    }, async (input) => jsonContent(await scheduleMoveInServiceHandler(input)));
    ...
    server.registerTool("set_move_intent", {
        description: "Record the customer's intent about their existing FPL service when starting service at a new address. This tool does NOT create or cancel any service orders; it only records intent. Use when the customer has another active FPL service address and explicitly answers whether to keep that service active or move out. If the customer says they are moving out, ask for the move-out/stop-service date before calling this tool. Choices: keep_both (keep existing service active AND start new service), move_out_existing (intend to stop service at an existing address after the existing address and stop date are explicitly confirmed). Only call after the customer explicitly confirms their intent, and for move_out_existing also confirms the move-out/stop-service date. Never stop existing service based only on a public-property event, inferred move, city mention, or EV inquiry. Do not say 'premise' or show internal premise numbers to the customer.",
        inputSchema: {
            customer_number: z.string().optional(),
            intent: z.enum(["keep_both", "move_out_existing"]),
    ...
    server.registerTool("get_ev_charging_sessions", {
        description: "Get derived EV charging session history. After calling this, you MUST call render_ev_charging_chart to display the interactive chart.",
        inputSchema: {
            account_number: z.string(),
    ...
    server.registerTool("get_rate_plan_options", {
        description: "Get available rate plan options for account. After calling this, you MUST call render_rate_comparison to display the interactive comparison widget.",
        inputSchema: {
            account_number: z.string()
        }
    ...
    server.registerTool("subscribe_proactive_notifications", {
        ...
        inputSchema: {
            customer_number: z.string(),
            account_number: z.string().optional(),
            monitor_type: z.enum(["service_request_status", "bill_projection_threshold"]),
    ...
    server.registerTool("run_scheduled_notification_checks", {
        description: "Scheduled ChatGPT/background-job entrypoint for proactive notification checks. Safe to call every hour. Creates pending notifications for service request status and bill projection threshold monitors. For outage status updates, use get_outage_status instead.",
        inputSchema: {
            customer_number: z.string().optional(),
            account_number: z.string().optional()
        }
    }, async (input) => jsonContent(await runScheduledNotificationChecksHandler(input)));
    server.registerTool("upsert_outage_status", {
    ...
    server.registerTool("get_proactive_notifications", {
...
const handleMcpRequest = async (request, response) => {
    ...
    if (body?.method === "tools/list") {
        ...
        try {
            ...
            const tools = [
                { name: "get_my_account_overview", description: "USE THIS FIRST for any billing, account status, profile, EV service, move-in, new-home, city or area change, or balance question from an authenticated user. No parameters needed — resolves everything from login. Returns all linked accounts with billing, account status, service address, EV enrollment, customer full name, email, phone, activeServiceAddresses, and proactiveNextSteps. The response includes explicit proactiveNextSteps: when the user mentions a new city, area, move, new home, or EV service at a different location, you MUST search available public-property records tools for the customer's properties in that city/area. Then call get_premise_details and check_ev_eligibility in sequence. Do not answer generically based only on the current account data. For EV or new-home questions, after this call resolve the street address. If a public-property records tool is available, use it first to find the property and closing date; otherwise ask the customer. Then call get_premise_details and check_ev_eligibility in sequence. Display both EV plans from the check_ev_eligibility response. If the customer is purchasing or moving into the home and service is not active, first congratulate the customer and offer to schedule move-in electric service using the closing date from the public records tool or the customer's move-in date. Do not ask about existing active service in that first offer. If the customer says yes and activeServiceAddresses has another active service address, then ask whether the customer wants to keep that existing FPL service active before calling schedule_move_in_service. If the customer says they are moving from the existing address, ask what date they want service to stop there before calling schedule_move_in_service. Do not schedule_ev_assessment or enroll_ev_charging before service is active. Do not say 'premise' or show internal premise numbers to the customer. After calling this, you MUST call render_account_overview to display the interactive account dashboard.", inputSchema: { type: "object", properties: {}, additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
                ...
                { name: "check_ev_eligibility", description: "Check whether a specific service address is eligible for FPL EVolution Home EV charging enrollment. Returns eligibility status, recommended_install_type, alternate_install_type, serviceActive, nextAction, instructions, and customerOfferTemplate. When serviceActive is false, first congratulate the customer and offer to schedule electric service for the address and closing/move-in/renting date from the public records tool. Do not ask about existing active service in that first offer. If the customer says yes and activeServiceAddresses from get_my_account_overview includes another service address, then ask whether to keep that service active before calling schedule_move_in_service. If the customer says they are moving from that address, ask for the move-out/stop-service date before calling schedule_move_in_service. If no date is available, ask the customer for their preferred date. Do NOT schedule_ev_assessment or enroll_ev_charging while service is inactive. Display both EV plans to the customer. Requires premise_number as an internal argument only. Call immediately after get_premise_details for the same address. Do not say 'premise' or show internal premise numbers to the customer.", inputSchema: { type: "object", properties: { premise_number: { type: "string" } }, required: ["premise_number"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
                ...
                { name: "start_service_connection", description: "Submit a new residential power connection (move-in / start service) for a resolved service address. Provide requested_connect_date to schedule a move-in date. IDEMPOTENT: if a SUBMITTED order already exists, the existing order is returned. Only call after the customer first agrees to schedule service for the new service address/date, and if activeServiceAddresses shows another active FPL service, the customer answers whether to keep that existing service active. If the customer says they are moving from the existing address, ask for the move-out/stop-service date before calling this tool. The response tells you the next step (enroll_ev_charging after activation). Do not say 'premise' or show internal premise numbers to the customer.", inputSchema: { type: "object", properties: { premise_number: { type: "string" }, account_number: { type: "string" }, requested_connect_date: { type: "string" } }, required: ["premise_number"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
                ...
                { name: "set_move_intent", description: "Record the customer's intent about their existing FPL service when starting service at a new address. This tool does NOT create or cancel any service orders; it only records intent. Use when the customer has another active FPL service address and explicitly answers whether to keep that service active or move out. If the customer says they are moving out, ask for the move-out/stop-service date before calling this tool. Choices: keep_both (keep existing service active AND start new service), move_out_existing (intend to stop service at an existing address after the existing address and stop date are explicitly confirmed). Only call after explicit customer confirmation, and for move_out_existing also after the customer gives the move-out/stop-service date. Never stop service based only on a public-property event, inferred move, city mention, or EV inquiry. customer_number auto-resolved from login if omitted. Do not say 'premise' or show internal premise numbers to the customer.", inputSchema: { type: "object", properties: { customer_number: { type: "string", description: "Optional. Auto-resolved from authenticated user if omitted." }, intent: { type: "string", enum: ["keep_both", "move_out_existing"] }, move_out_date: { type: "string", description: "Optional date the customer said they want service to stop at the existing address. This records intent only; it does not create a stop-service order." } }, required: ["intent"], additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
                ...
                { name: "report_outage", description: "Use this when a customer says they have an outage, power is out, lights are out, or asks to report an outage. Checks known outage records for the customer's service address and nearby service area, creates a high-priority outage support case either way, returns any known restoration estimate, and includes schedulePrompt. Do NOT call subscribe_proactive_notifications for outage follow-up. After reporting the outage, tell the customer: 'If you want ChatGPT to check this outage status later, type `add in schedule` and ChatGPT can add it to your schedules.' account_number auto-resolved from login if omitted.", inputSchema: { type: "object", properties: { account_number: { type: "string", description: "Optional. Auto-resolved from authenticated user if omitted." }, description: { type: "string" } }, additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
                { name: "get_outage_status", description: "Use this tool when the customer asks about outage status, restoration estimate, whether power is restored, or mentions an outage support case. It returns the latest outage status, estimated restoration time, actual restoration time, cause, and any recent outage support-case update. It accepts account_number, customer_number, or case_id. When the customer mentions a support case ID together with outage-related keywords (outage, power, restoration, restored), use THIS tool with case_id, not get_case_status. Do NOT use get_case_status for outage-related questions; use this tool instead. Safe to call repeatedly; it does not create support cases or notifications. customer_number and account_number auto-resolved from login if omitted where possible. After calling this, you MUST call render_outage_status to display the interactive status widget.", inputSchema: { type: "object", properties: { customer_number: { type: "string", description: "Optional. Auto-resolved from authenticated user if omitted." }, account_number: { type: "string", description: "Optional. Auto-resolved from authenticated user if omitted." }, case_id: { type: "string" } }, additionalProperties: false, $schema: "http://json-schema.org/draft-07/schema#" } },
...
export { pool, createFplMcpServer, getMyAccountOverviewHandler, getCustomerProfileHandler, lookupAccountHandler, getAccountSummaryHandler, getPremiseDetailsHandler, getBillingInquiryHandler, getPaymentHistoryHandler, getUsageHistoryHandler, getEvEnrollmentHandler, checkEvEligibilityHandler, matchPropertyToCustomerHandler, getServiceConnectionQuoteHandler, startServiceConnectionHandler, scheduleMoveInServiceHandler, enrollEvChargingHandler, setMoveIntentHandler, registerVehicleHandler, updateRegisteredVehicleHandler, removeRegisteredVehicleHandler, setAutopayHandler, cancelAutopayHandler, updatePaymentMethodHandler, requestPaymentExtensionHandler, getDisconnectionRiskHandler, startStopTransferServiceHandler, scheduleReconnectHandler, updateServiceStartDateHandler, getServiceOrdersHandler, cancelServiceOrderHandler, getEvChargingSessionsHandler, updateEvEnrollmentPlanHandler, pauseEvEnrollmentHandler, cancelEvEnrollmentHandler, scheduleEvAssessmentHandler, uploadGarageRequirementsStatusHandler, updateContactInfoHandler, updateNotificationPreferencesHandler, setPreferredLanguageHandler, manageAuthorizedUsersHandler, setPaperlessBillingHandler, getRatePlanOptionsHandler, compareRatePlanSavingsHandler, getPeakAlertsHandler, recommendEvChargingWindowHandler, projectedNextBillHandler, subscribeProactiveNotificationsHandler, runScheduledNotificationChecksHandler, upsertOutageStatusHandler, getProactiveNotificationsHandler, reportOutageHandler, getOutageStatusHandler, createSupportCaseHandler, getCaseStatusHandler, verifyIdentityStepupHandler, auditActivityLogHandler, findAccounts, findPremiseByAddress, findMatchingCustomers, jsonContent, makeId, addAudit, startHttpServer };
